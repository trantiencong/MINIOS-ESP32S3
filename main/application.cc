...
const char* Application::MapWeatherIconCode(const std::string& icon_code) {
    ...
    if (icon_code == "02n")
        return FONT_AWESOME_CLOUD_MOON;
    if (icon_code == "03d" || icon_code == "03n" || icon_code == "04d" || icon_code == "04n")
        return FONT_AWESOME_CLOUD;
    if (icon_code == "09d" || icon_code == "09n")
        return FONT_AWESOME_CLOUD_SHOWERS_HEAVY;
...
void Application::UpdateIdleInfo() {
    ...
    display->SetIdleInfo(weather_info_.location.c_str(), weather_info_.description.c_str(),
                         weather_info_.temperature.c_str(), weather_info_.humidity.c_str());
}
